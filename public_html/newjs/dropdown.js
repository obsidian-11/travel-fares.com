...
$(document).ready(function () {
    ...
    $('.count').click(function (e) {
        ...
        var btn = $(this),
            ...
            newVal = 0, minVal = 0;
        var adt = parseInt($('#AdultsRT').val(), 10), chd = parseInt($('#ChildrenRT').val(), 10), inf = parseInt($('#InfantsST').val(), 10);
        ...
        if (((adt + chd + inf) >= 9 || (inf >= adt && nm == 'InfantsST')) && button_classes.indexOf('up_count') !== -1) { }
        else if (inf == adt && button_classes.indexOf('down_count') !== -1 && nm == 'adults') { }
        else {
    ...
    $.ajax({
            ...
            success: function (response) {
                var h = '', _offrs = [];
                try {
                    _offrs = JSON.parse(response.d.substring(1, response.d.length - 2) + '}').Table;
                }
                catch (e) { }
                $('div[data-fr]').each(function () {
                    var _dfrom = $(this).attr('data-fr');
                    var _offr = $.grep(_offrs, function (n, i) { return _dfrom.indexOf(n.destfrom) >= 0 });
                    h = ''; var cnt = 0;
                    $.each(_offr, function () {
                        if (cnt < 9) {
                            h += '<div class="col-lg-4 col-md-6 col-sm-12 mb-4">';
                            h += '<div style="cursor:pointer;" onclick="sbmt(\'' + this.destfrom + '\',\'' + this.destto + '\',\'' + this.ddate + '\',\'' + this.rdate + '\');">';
                            h += '<div class="ft-box">';
                            h += '<div class="row">';
                            h += '<div class="col-4">';
                            h += '<span class="pl-name ex" data-bs-toggle="tooltip" data-bs-placement="top" title="' + this.destfromname + '">' + this.destfromname + '</span>';
                            h += '<span class="date-sm ex">' + this.ddate + '</span>';
                            h += '</div>';
                            h += '<div class="col-1 text-center ps-0 pe-0">';
                            h += '<span class="round-icon"><svg xmlns="http://www.w3.org/2000/svg" width="18px" height="18px" viewBox="0 0 512 512"><title>Swap Horizontal</title><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="32" d="M304 48l112 112-112 112M398.87 160H96M208 464L96 352l112-112M114 352h302"/></svg></span>';
                            h += '</div>';
                            h += '<div class="col-4">';
                            h += '<span class="pl-name ex" data-bs-toggle="tooltip" data-bs-placement="top" title="' + this.desttoname + '">' + this.desttoname + '</span>';
                            h += '<span class="date-sm ex">' + this.rdate + '</span>';
                            h += '</div>';
                            h += '<div class="col-3 border-start">';
                            h += '<span class="price-rate">$' + Math.floor(parseFloat(this.total, 10)) + '</span>';
                            h += '</div>';
                            h += '</div>';
                            h += '</div>';
                            h += '</div>';
